...
export const Fertilizer = () => {
  ...
  const openFertilizerModal = () => setShowFertilizerModal(true);
  const closeFertilizerModal = () => setShowFertilizerModal(false);
    ...
    useEffect(() => {
      ...
      AxiosInstance.get('/api/fertilizer/')
        .then(response => {
          ...
          setFertilizerData(fetchedData); // Set the fetched data into the state
    ...
    const sortedData = [...fertilizerData].sort((a, b) => {
    ...
    const handleAccept = (id) => {
      AxiosInstance.post(`/api/fertilizer/approve`, { id })
        .then(response => {
          // Update the egg data after the status change
          setFertilizerData(prevData => prevData.map(entry =>
            entry.id === id ? { ...entry, status: 'Accepted' } : entry
          ));
          navigate(0);  // Reload page after update
        })
        .catch(error => {
          console.error('Error approving fertilizer collection:', error);
        });
    };
  
    const handleDecline = (id) => {
      AxiosInstance.post(`/api/fertilizer/decline`, { id })
        .then(response => {
          // Update the egg data after the status change
          setFertilizerData(prevData => prevData.map(entry =>
        ...
        .catch(error => {
          console.error('Error declining fertilizer collection:', error);
        });
    };
    ...
    const refreshData = () => {
      AxiosInstance.get('/api/fertilizer/')
        .then(response => {
          ...
          setFertilizerData(fetchedData);
        })
        .catch(error => {
          console.error('Error refreshing fertilizer data: ', error);
        });
    };
              ...
              useEffect(() => {
                ...
                const filtered = fertilizerData.filter((entry) => {
                  const entryDate = new Date(entry.date);
